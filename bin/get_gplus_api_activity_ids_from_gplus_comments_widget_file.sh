...
if hash pup 2>/dev/null; then #If `pup` is installed, use that, else fall back to gsed
  ...
  results="$(cat "$blog_comments_widget_file" | pup 'script[nonce]:contains("cw.gfr") text{}'| gnused 's/^AF_initDataCallback\(.+data://' | gnused 's/^}\);$//' | jq -r --slurp '[.[0]? | .[]? | .[]? | .[]? | .[]? | .[]?] | .[ range(1;length;3) ]' | unsorted_uniques)"
else
